...
import { StarIcon as OutlineStarIcon, TagIcon, GlobeAltIcon, PhoneIcon, UsersIcon, UserCircleIcon, SparklesIcon, DocumentTextIcon, CheckCircleIcon, XCircleIcon, ChevronDownIcon, ArrowPathIcon } from '@heroicons/react/24/outline';
...
export default function ShareRecommendation() {
  ...
  const fetchUserTrustCircles = useCallback(async () => {
    ...
    try {
      const myCommRes = await fetch(`${API_URL}/api/communities/user/${userEmail}/communities`);
      if (!myCommRes.ok) {
        const errorData = await myCommRes.json().catch(() => ({ message: 'Failed to fetch your trust circles.' }));
        throw new Error(errorData.message || 'Failed to fetch your trust circles.');
      }
      ...
      if (Array.isArray(myCommData)) {
          ...
          for (const community of myCommData) {
              if (community && community.id && !seenIds.has(community.id)) {
                  uniqueMyComms.push({id: community.id, name: community.name});
      ...
      setTrustCirclesError(err.message || 'Could not load your trust circles.');
      setUserTrustCircles([]);
    } finally {
  ...
  const requiredFieldsComplete = businessName.trim() && recommendationBlurb.trim() && rating > 0;
  ...
  const handlePublishScopeChange = (e) => {
    ...
    if (newScope !== 'Specific Trust Circles') {
      ...
      if (userTrustCircles.length === 0 && !trustCirclesLoading && !trustCirclesError) {
        fetchUserTrustCircles();
      }
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    if (!requiredFieldsComplete) {
      setMessage('error:Please fill out all required fields: Service Provider Name, Your Experience, and Your Rating.');
      return;
    }
    if (publishScope === 'Specific Trust Circles' && selectedTrustCircles.length === 0) {
      setMessage('error:Please select at least one specific trust circle to share with, or choose a different publish scope.');
      return;
    }
    ...
    try {
      ...
      setMessage("success:Recommendation submitted successfully! Redirecting...");
        ...
        <div className={`optional-section-wrapper ${requiredFieldsComplete ? 'visible' : ''}`}>
          ...
          <section className="form-section optional-section">
            ...
            <div className="form-grid">
              ...
              <div className="form-group"><label htmlFor="website"><GlobeAltIcon/> Website (Referral link, if applicable)</label><input id="website" type="url" placeholder="https://provider.com" value={website} onChange={(e) => setWebsite(e.target.value)} className={website ? 'has-value' : ''} /></div>
              ...
              <div className="trust-circle-select-wrapper">
                ...
                {trustCirclesLoading && (
                  <div className="loading-trust-circles">
                    <ArrowPathIcon className="animate-spin h-5 w-5 mr-2 inline" /> Loading your communities...
                  </div>
                )}
                {trustCirclesError && !trustCirclesLoading && (
                  <div className="error-trust-circles">
                    <XCircleIcon className="h-5 w-5 mr-2 inline text-red-500" /> {trustCirclesError}
                    <button onClick={fetchUserTrustCircles} className="retry-button ml-2">Retry</button>
                  </div>
                )}
                {!trustCirclesLoading && !trustCirclesError && userTrustCircles.length > 0 && (
                ...
                {!trustCirclesLoading && !trustCirclesError && userTrustCircles.length === 0 && (
                  <p className="no-trust-circles-message">You are not part of any communities, or we couldn't load them. <a href="/trustcircles" onClick={(e) => { e.preventDefault(); navigate('/trustcircles');}} className="text-blue-600 hover:underline">Manage your Trust Circles</a> or try again.</p>
                )}
              </div>
            )}
          </section>
...
//           for (const community of myCommData) {
//               if (community && community.id && !seenIds.has(community.id)) {
//                   uniqueMyComms.push({id: community.id, name: community.name}); // Store only id and name
//                   seenIds.add(community.id);
//               }
//           }
//           setUserTrustCircles(uniqueMyComms);
...
//     } else {
//       if (userTrustCircles.length === 0 && !trustCirclesLoading && !trustCirclesError) {
//         fetchUserTrustCircles(); // Fetch if not already fetched and scope changes to specific
//       }
//     }
//   };
...
//             )}
//           </section>
//           <div className="button-row"><button type="submit" className="btn btn-primary" disabled={isSubmitting || !requiredFieldsComplete}>{isSubmitting ? 'Sharing...' : 'Share Recommendation'}<CheckCircleIcon /></button></div>
//         </div>
//         {message && (<div className={`message ${message.startsWith('error') ? 'error' : 'success'} ${message ? 'visible' : ''}`}>{message.startsWith('error') ? <XCircleIcon /> : <CheckCircleIcon />}<span>{message.substring(message.indexOf(':') + 1)}</span></div>)}
//       </form>
//     </>

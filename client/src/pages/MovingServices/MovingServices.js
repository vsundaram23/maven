 ...
 useEffect(() => {
  ...
  useEffect(() => {
    if (!currentUserId && !localStorage.getItem('user')) {
        setError("Please log in to view moving service providers.");
        setLoading(false);
        setProviders([]);
        return;
    }
    ...
    const getProviders = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${API_URL}/api/movingProviders?user_id=${currentUserId}`);
        if (!response.ok) {
        ...
        if (!data.success) {
            throw new Error(data.message || "Failed to fetch moving providers successfully");
        }
    ...
    <div className="moving-services-container">
      ...
      {!loading && !error && providers.length === 0 && (
        <div className="no-providers-message">
          ...
          <p>
            We couldn't find any moving service recommendations visible to you right now.
          </p>

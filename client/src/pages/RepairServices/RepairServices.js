...
const ProviderProfileModal = ({ isOpen, onClose, provider, reviews = [], setSelectedProvider, setIsReviewModalOpen }) => {
  ...
  const alsoUsedBy = Array.from(recommenders).filter(name => name !== provider.recommender_name);
      ...
      <div className="profile-modal-content">
        ...
        <div className="profile-section">
          ...
          {provider.recommender_name && (
            <p><strong>Recommended by:</strong> {provider.recommender_name}</p>
          )}
          {alsoUsedBy.length > 0 && (
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const getProviders = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${API_URL}/api/repairProviders?user_id=${currentUserId}`);
        if (!response.ok) {
        ...
        if (!data.success) {
            throw new Error(data.message || "Failed to fetch repair providers successfully");
        }
  ...
  return (
    <div className="repair-services-container">
      <h1 className="section-heading">Top Home Renovation Service Providers</h1>
      ...
      {!loading && !error && providers.length === 0 && (
        <div className="no-providers-message">
          ...
          <p>
            We couldn't find any repair service recommendations visible to you right now.
          </p>
...
//         const filtered = data.filter(p => p.service_type === 'Structural Repairs');

//         const statsMap = {};
//         const allReviewsMap = {};

//         await Promise.all(filtered.map(async (provider) => {
//           try {
//             const statsRes = await fetch(`${API_URL}/api/reviews/stats/${provider.id}`);
//             const statsData = await statsRes.json();
//             statsMap[provider.id] = {
...
//       try {
//         const data = await fetchRepairProviders();
//         setProviders(data); // no filtering now
//       } catch (err) {
//         setError('Failed to fetch providers');
...
//   if (loading) return <div className="loading-spinner">Loading...</div>;
//   if (error) return <div className="error-message">Error: {error}</div>;
//   if (!providers.length) return <div className="no-data">No repair service providers found</div>;

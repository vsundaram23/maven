...
const RepairServices = () => {
        ...
        const getProviders = async () => {
            ...
            try {
                ...
                const response = await fetch(
                    `${API_URL}/api/repairProviders?${params.toString()}`
                );
    ...
    const providers = useMemo(() => {
        ...
        return list.sort((a, b) => {
            const da = a.date_of_recommendation;
            const db = b.date_of_recommendation;
    ...
    return (
        <div className="repair-services-container">
            <h1 className="section-heading">
                Top Home Renovation Service Providers
            </h1>
                    ...
                    {showCityFilter && (
                        <div className="profile-city-filter-wrapper">
                            <div className="profile-city-filter-checkboxes">
                                {availableCities.map(([cityName, count]) => (
                                    <div key={cityName} className="profile-city-checkbox-item">
                                        <input
                                            type="checkbox"
                                            id={`city-${cityName.replace(/\s+/g, '-')}`}
                                            name={cityName}
                                            checked={selectedCities.includes(cityName)}
                                            onChange={() => handleCitySelection(cityName)}
                                        />
                                        <label
                                        ...
                                        <span className="profile-city-count">({count})</span>
                                    </div>
                                ))}
                                {selectedCities.length > 0 && (
                                    <button

...
const UtilitiesServices = () => {
  ...
  useEffect(() => {
    const getProviders = async () => {
      try {
        ...
        setProviders(data); // no filtering
      } catch (err) {
  ...
  if (!providers.length) return <div className="no-data">No utilities service providers found</div>;

  return (
    <div className="utilities-services-container">
      <h1 className="section-heading">Top Utilities Service Providers</h1>
      <ul className="provider-list">
        {providers.map((provider) => (
          <li key={provider.id || Math.random()} className="provider-card">
            <div className="card-header">
              <h2 className="card-title">{provider.business_name}</h2>
              <span className="badge">Utilities Services</span>
            </div>

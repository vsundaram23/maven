...
import { MapPinIcon, LockClosedIcon, XMarkIcon, MagnifyingGlassIcon } from '@heroicons/react/24/solid';
import './Home.css';
...
const Home = () => {
  ...
  const handleSearch = async (e) => {
    ...
    setIsSearching(true);
    const searchUrl = `${API_URL}/api/providers/search?q=${encodeURIComponent(q)}&user_id=${currentUserId}&location=${encodeURIComponent(LOCKED_LOCATION)}`;
    ...
    try {
      ...
      if (d.success) {
        const base = `/search?q=${encodeURIComponent(q)}&location=${encodeURIComponent(LOCKED_LOCATION)}`;
        navigate(

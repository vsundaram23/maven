...
import { FaEnvelope, FaPhone, FaPlusCircle, FaStar, FaUsers } from 'react-icons/fa';
 ...
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const getProviders = async () => {
      ...
      setError(null);
      try {
        const response = await fetch(`${API_URL}/api/outdoorProviders?user_id=${currentUserId}`);
        if (!response.ok) {
    ...
    <div className="outdoor-services-container">
      ...
      {!loading && !error && providers.length === 0 && (
        <div className="no-providers-message">
          ...
          <p>
            We couldn't find any outdoor service recommendations visible to you right now.
          </p>

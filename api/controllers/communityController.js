...
const createCommunity = async (name, description, created_by_clerk_id) => {
  ...
  try {
    ...
    const userRes = await client.query('SELECT id FROM users WHERE clerk_id = $1', [created_by_clerk_id]);
...
const getCommunityDetails = async (communityId, clerkUserId) => {
  ...
  try {
    ...
    if (communityRes.rows.length === 0) throw new Error('Community not found');
    ...
    if (internalUserUuid) {
      ...
      if (membershipRes.rows.length > 0) community.currentUserStatus = membershipRes.rows[0].status;
    }
    return community;
  } catch (error) {
    if (error.message === 'Community not found') throw error;
    throw new Error('Database error fetching community details: ' + error.message);
  } finally {
    if(client) client.release();
  }
};
...
const requestToJoinCommunity = async (clerkUserId, community_id) => {
  ...
  try {
    ...
    const existing = await client.query(`SELECT status FROM community_memberships WHERE user_id = $1 AND community_id = $2`, [internalUserUuid, community_id]);
    if (existing.rows.length > 0) {
      if(existing.rows[0].status === 'approved') throw new Error('Already a member of this community.');
      if (existing.rows[0].status === 'requested') throw new Error('Already requested to join this community.');
    }
    const result = await client.query(
    ...
    if (error.message.startsWith('Already') || error.message === 'User not found.') throw error;
...
const getJoinRequests = async (communityId, clerkCreatorId) => {
  ...
  try {
    ...
    const userRes = await client.query('SELECT id FROM users WHERE clerk_id = $1', [clerkCreatorId]);
    if (userRes.rows.length > 0) {
    ...
    const checkCreator = await client.query(`SELECT 1 FROM communities WHERE id = $1 AND created_by = $2`, [communityId, internalCreatorUuid]);
    if (checkCreator.rows.length === 0) throw new Error('Not authorized to view join requests for this community.');
    
    const result = await client.query(
      `SELECT cm.user_id, cm.community_id, cm.status, cm.requested_at, u.name, u.email, u.clerk_id
       FROM community_memberships cm
       JOIN users u ON u.id = cm.user_id
       WHERE cm.community_id = $1 AND cm.status = 'requested' ORDER BY cm.requested_at ASC`, [communityId]
    );
    return result.rows;
  } catch (error) {
    if (error.message.startsWith('Not authorized') || error.message === 'Creator user not found.') throw error;
...
const approveMembership = async (community_id, clerkNewUserId, clerkApproverId) => {
    const client = await pool.connect();
    try {
      await client.query('BEGIN');
      let internalNewUserUuid = null;
      let internalApproverUuid = null;

      const approverRes = await client.query('SELECT id FROM users WHERE clerk_id = $1', [clerkApproverId]);
      if (approverRes.rows.length > 0) internalApproverUuid = approverRes.rows[0].id;
      else throw new Error('Approver user not found.');

      const newUserRes = await client.query('SELECT id FROM users WHERE clerk_id = $1', [clerkNewUserId]);
      if (newUserRes.rows.length > 0) internalNewUserUuid = newUserRes.rows[0].id;
      else throw new Error('User to be approved not found.');

      const communityCheck = await client.query(`SELECT created_by FROM communities WHERE id = $1`,[community_id]);
      if (communityCheck.rows.length === 0) throw new Error('Community not found.');
      if (communityCheck.rows[0].created_by !== internalApproverUuid) throw new Error('User not authorized to approve memberships.');
      ...
      if (membershipUpdate.rowCount === 0) throw new Error('No pending request found or already approved.');
      ...
      await client.query('ROLLBACK');
      if (error.message.includes('Not authorized') || error.message.includes('No pending request') || error.message.includes('not found')) throw error;
      throw new Error('Database error approving membership: ' + error.message);
    } finally {
      client.release();
    }
};
const getCommunityMembers = async (communityId) => {
  ...
  try {
    client = await pool.connect();
    const result = await client.query(
      `SELECT u.id, u.clerk_id, u.name, u.email, u.profile_image 
       FROM users u
    ...
    throw new Error('Database error fetching community members: ' + error.message);
  } finally {
    if (client) client.release();
  }
};

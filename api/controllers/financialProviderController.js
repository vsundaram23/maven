...
const getVisibleProvidersBaseQueryForFinancialPage = (currentUserId) => {
    const query = `
    SELECT DISTINCT
        sp.id,
        sp.business_name,
...
const getAllVisibleFinancialProviders = async (req, res) => {
    ...
    if (!clerkUserId || !userEmail) {
        return res.status(400).json({
            success: false,
            message:
                "User ID and email are required to fetch financial service providers.",
        });
    }
    ...
    try {
        ...
        const { query: baseQuery, queryParams } =
            getVisibleProvidersBaseQueryForFinancialPage(internalUserId);
        const serviceName = "Financial Services";
        const finalQuery = `
        ...
        console.error("Database error fetching financial providers:", err);
        res.status(500).json({
            success: false,
...
const getVisibleFinancialProviderById = async (req, res) => {
    ...
    if (!clerkUserId || !userEmail) {
        return res.status(400).json({
            success: false,
            message:
                "User ID and email are required to fetch financial provider details.",
        });
    }
    ...
    try {
        ...
        const finalQuery = `
            ...
            AND VisibleProvidersCTE.category_name = $${paramIndexForService};
        `;
        ...
        if (result.rows.length === 0) {
            return res.status(404).json({
                success: false,
                message:
                    "Financial provider not found or not accessible to this user.",
            });
        }
        ...
        console.error(
            "Database error fetching specific financial provider:",
            err
        );
        res.status(500).json({
            success: false,
